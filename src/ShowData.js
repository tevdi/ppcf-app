...
class ShowData extends React.Component {
  ...
  stringFound(element) {
    if (this.state.filter === '') {
      ...
      return !(
        element.title.toLowerCase().indexOf(this.state.filter) === -1 &&
        element.type.toLowerCase().indexOf(this.state.filter) === -1
      );
    }
  }
    ...
    fetch(`${process.env.API_REST_URL}/rest/data`)
      ...
      .then((data) => {
        if (this.state.filter != '' || this.state.checkedCategories.length != 0) {
          ...
          data.filter((element) => {
            if (
              (this.typeIsChecked(element.typeid) && this.state.filter === '') ||
              (this.typeIsChecked(element.typeid) && this.stringFound(element)) ||
              (this.state.checkedCategories.length == 0 && this.stringFound(element))
            ) {
        ...
        fetch(`${process.env.API_REST_URL}/rest/data/getCategories`)
          // fetch(`${process.env.BASENAME}/categories.json`)
          .then((response) => {
            return response.json();
          })
          .then((categories) => {
        ...
        <div className="container">
          ...
          <div id="mainData">
            <div id="show-data" className="col-lg-10">
              <Link to={`${process.env.BASENAME}/add-element`} className="btn btn-info">
                Add element
              </Link>
              <div>
                {' '}
                Total elements: <b>{this.state.totalElements}</b>
              </div>
              <div className="elementsRows">
                {this.state.partialElementsList.map((element, idx) => (
                  <div key={idx} className="container-fluid">
                    <div className="row">
                      <div className="col-lg-2">{this.extractDate(element.date)}</div>
                      <div className="col-lg-3">{element.type}</div>
                      <div className="col-lg-7 desc">
                        <Link to={`${process.env.BASENAME}/element/${element.id}`}>{element.title}</Link>
                      </div>
                    </div>
                  </div>
                ))}
            ...
            <aside className="col-lg-2">
              <button className="btn btn-primary btnToggleFilterDiv" onClick={this.handleFilterDiv.bind(this)}>
                Filter
              </button>
